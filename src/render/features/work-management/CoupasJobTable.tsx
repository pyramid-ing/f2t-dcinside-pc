...
const CoupasJobTable: React.FC = () => {
  ...
  const handleStatusChange = async (job: any, value: JobStatus) => {
  ...
  const handleScheduledAtChange = async (job: any, date: dayjs.Dayjs | null) => {
    try {
  ...
  if (!hasCommentPermission) {
    return (
      <Alert
        message="권한이 없습니다"
        description="쿠파스 워크플로우 작업 관리 기능을 사용하려면 '댓글작성' 권한이 필요합니다. 라이센스를 추가로 구매하셔야합니다."
        type="warning"
        showIcon
      />
    )
  ...
  return (
    <div>
      ...
      <Table
        ...
        rowClassName={(record: any) => `row-${record.status}`}
        columns={[
          {
            title: (
          ...
          },
          {
            ...
            render: (title: string, row: any) => {
              const postUrl = row.coupasJob?.postUrl || '-'
          ...
          },
          {
            ...
            render: (text: string, row: any) => {
              const wpUrl = row.coupasJob?.wordpressUrl || '-'
              return (
                <span title={wpUrl} style={{ fontSize: '13px' }}>
                  {wpUrl}
                </span>
              )
            },
            ...
            render: (v: string, row: any) => {
              ...
              const popoverContent = (
                <PopoverContent>
                  ...
                  {row.coupasJob?.resultBlogLink && (
                    <div className="result-url">
                      <a
                        href={row.coupasJob.resultBlogLink}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={e => {
                          e.preventDefault()
                          window.electronAPI?.openExternal(row.coupasJob.resultBlogLink)
                        }}
                      >
                        블로그 링크: {row.coupasJob.resultBlogLink}
                      </a>
                    </div>
                  )}
                </PopoverContent>
          ...
          },
          {
            ...
            dataIndex: 'status',
            key: 'status',
            render: (value: JobStatus, record: any) =>
